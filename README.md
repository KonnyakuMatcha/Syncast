...
4. 屏幕默认采用房主到观众的星型 P2P；房主也可以开启动态树状转发。语音始终采用成员间 mesh。
...
房主可以在通话频道标题旁开启“树状”。开启后，画面和共享来源音频使用最多三叉、最多两层的浅层树；房主通常只向三个第一层成员发送媒体，第一层成员再转发给叶节点。识别为移动设备的客户端只作为叶节点。成员加入、离开或中继连接失败时，房主重新发布带版本的拓扑。子节点会通过 WebRTC 统计检查实际选中的 ICE 候选对；如果某条边使用了 TURN，会先尝试更换父节点，候选父节点都尝试过后才保留 TURN 作为可用性兜底。关闭开关会立即恢复房主到所有观众的星型连接。
